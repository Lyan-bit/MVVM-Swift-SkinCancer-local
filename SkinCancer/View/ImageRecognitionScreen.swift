    ...
    var body: some View {
  	NavigationView {
        ...
        VStack {
            ...
            HStack (spacing: 20) {
                Button(action: {result = self.recognition.imageRecognition(x: objectId) } ) { Text("Classify") }
				Button(action: {self.recognition.cancelImageRecognition() } ) { Text("Cancel") }
...
struct ImageRecognitionScreen_Previews: PreviewProvider {
    static var previews: some View {
        ImageRecognitionScreen(recognition: ImageRecognitionModel.getInstance(), model: SkinViewModel.getInstance())
    }
}
